              ...
              {Object.entries(groupSubOrders()).flatMap(([baseOrder, { main, subs, boxCount }]) => {
                ...
                return [
                  <tr key={baseOrder} className="main-order">
                    ...
                    <td className="customer-cell">{main.customerName}</td>
                    <td className="phone-cell">{main.phone}</td>
                    <td>
                      <div className="delivery-cell">
                        <span className="delivery-date">{main.deliveryDate}</span>
                  ...
                  ...(hasMultipleBoxes && expanded ? subs.map((sub, i) => (
                    <tr key={`${baseOrder}-${sub.subLabel}`} className="sub-order">
                      <td title={`Box ${i + 1}: ${sub.box.items.map(it => `${it.name} x${it.qty}`).join(', ')}`}>
                        &nbsp;&nbsp;&nbsp;&nbsp;↳ {baseOrder}{sub.subLabel}
                      </td>
                      <td className="customer-cell">{sub.customerName}</td>
                      <td className="phone-cell">{sub.phone}</td>
                      <td>
                        <div className="delivery-cell">
                          <span className="delivery-date">{sub.deliveryDate}</span>
