...
const OrderForm = React.forwardRef(({ selectedOrder, setSelectedOrder }, ref) => {
  ...
  if (editingOrderId || !currentUser.branch || currentUser.branch === 'Loading...') return;
...
const activeBranch = currentUser.role === 'admin' && selectedBranch 
  ...
  const handleSubmit = async (event, status = 'saved') => {
    ...
    const orderData = {
      ...
      branchCode: branchPrefixes[currentUser.branch],
      ...
      balance: calculateGrandTotal() - advancePaid,
      status: status
    };
...
useEffect(() => {
  ...
  }, [orderInfo, boxes, notes, extraDiscount, advancePaid, customer, selectedOrder, editingOrderId, currentUser]);
